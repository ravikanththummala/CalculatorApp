...
struct KeyView: View {
        ...
        VStack{
            ...
            HStack {
                RoundedRectangle(cornerRadius: 15)
                    .foregroundColor(changeColor ? Color("numKeys").opacity(0.4) : Color.pink.opacity(0.2))
                    .scaleEffect(changeColor ? 1.5 : 1.0)
                    .frame(width: 350,height: 280)
                    .animation(.easeOut.speed(0.17).repeatForever(),value: changeColor)
                    .onAppear(perform: {
                    ...
                    ForEach(row,id: \.self){ column in
                        Button {
                            ...
                            Text(column.rawValue)
                                .font(.system(size: 30))
                                .frame(width: getwidth(elem: column), height: getHeight(elem: column))
                                .background(column.buttonColor)
                                .foregroundColor(.black)
                                .cornerRadius(getwidth(elem: column)/2)
    ...
    func getwidth(elem:Keys) -> CGFloat{
        if elem == .zero {
            return (UIScreen.main.bounds.width - (2*10)) / 2
        }else{
